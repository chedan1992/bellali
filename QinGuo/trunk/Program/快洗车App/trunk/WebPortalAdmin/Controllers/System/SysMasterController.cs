...
namespace WebPortalAdmin.Controllers
{
    ...
    public class SysMasterController : BaseController<ModSysMaster>
    {
        ...
        public void SaveData(ModSysMaster t)
        {
            ...
            t.Attribute = (int)AdminTypeEnum.系统管理员;//栏目类型
        ...
        public void DeleteData(string id)
        {
            ...
            try
            {
                string sql = "update Sys_Master set Status=" + (int)StatusEnum.删除 + " where Id in(" + id + ")";
                if (new BllSysMaster().ExecuteNonQueryByText(sql) > 0)
                {
                    msg.success = true;
                    LogInsert(OperationTypeEnum.操作, "用户删除", "用户删除成功.");
                }
                else
                {
                    msg.success = false;
                    msg.msg = "操作失败";
                    LogInsert(OperationTypeEnum.操作, "用户删除", "用户删除失败.");
                }
            }
            catch (Exception ex)
            {
