var selectNode = null;//存储编辑选中的主表记录
//单位公司
Ext.onReady(function () {
    ...
    var store = GridStore(Member, '/CACAI/InComeBill/SearchData', className, 'FinancialState', 'asc');
    ...
    var grid = new Ext.grid.EditorGridPanel({
        ...
        //                items: [
        //                        {
        //                            icon: '../../Resource/css/icons/toolbar/GTP_upload.png', // 指定图标
        //                            tooltip: '上传',
        ...
        //               },
             ...
             renderer: function (value, meta, record, rowIdx, colIdx, store) {
                 //var content = '<div id="UpLoad' + record.data.InNumber + '"><span style="vertical-align: middle;cursor:pointer" title="上传凭证"><img style="vertical-align: middle; width:18px; height:18px;border:0" class="GTP_upload"  src="../../Content/Extjs/resources/images/default/s.gif"/></span></div>';
                 var content = '<form style="cursor:pointer;" id="upForm' + record.data.Id + '">' +
                                    '<span style="vertical-align: middle;cursor:pointer" title="上传凭证"><img style="vertical-align: middle; width:18px; height:18px;border:0" class="GTP_upload"  src="../../Content/Extjs/resources/images/default/s.gif"/></span>' +
                                    '<input type="file" name="file1" style="width:30px;cursor:pointer;position:absolute; z-index:100; margin-left:-30px;opacity:0;filter:alpha(opacity=0); margin-top:-5px;" onchange="UoLoad(\'' + record.data.Id + '\');">' +
                                '</form> ';
                 return content;
             }
         },
             { header: "凭证", dataIndex: 'HasFileAttach', menuDisabled: true, sortable: true, width: 40, align: 'center',
                 renderer: function (value, cellmeta, record, rowIndex, columnIndex, store) {
                     if (value == true) {
                         return '<a title="附件详情" href="#" onclick="FileAttach()"><img alt="凭证详情" style="vertical-align: middle; width:15px; height:15px;border:false" src="../../Resource/css/icons/toolbar/GTP_accessory.png"/>&nbsp;</a>';
                     }
                 }
             },
           {
        ...
        },
        {
            ...
            editor: {
                ...
                listeners: {
                    blur: function (a, b, c) {
                        //SaveMain();
                        // Ext.getCmp("gg").getView().getCell(1, 4).style.backgroundColor = '#B4EEB4';
                    }
                }
            },
            renderer: function (data, metadata, record, rowIndex, columnIndex, store) {
        ...
        listeners: {
            'beforeedit': function (o) {
                var FinancialState = o.record.get('FinancialState');
                if (o.field == "Remark") {
    ...
    grid.store.on('beforeload', function (node) {
        ...
        grid.getStore().proxy.conn.url = '/CACAI/InComeBill/SearchData?' + search;
    });
    ...
    var rightstore = GridStore(Human, '/CACAI/InComeBill/SearchDataDetail?MainId=' + treeNodeId, '', 'StyleNum', 'asc', false, true);
    ...
    rightGrid.store.on('beforeload', function (node) {
        //判断是显示当前供应商还是全部
        rightGrid.getStore().proxy.conn.url = '/CACAI/InComeBill/SearchDataDetail?MainId=' + treeNodeId;
    });
    //viewport
...
//按钮上传图片
function showBtnImg(response) {
    var json = $.parseJSON(response);
    if (json.Status == 1) {
        var val = json.Msg; //图片内容
...
function upImageMore(buttonObj, savefilepath, callbakfun) {
    if (buttonObj.length > 0) {
        var intervalTime = null;
        var urlstr = "/Common/UpImage";
        var upload = new AjaxUpload(buttonObj, {
            action: urlstr,
            name: 'userFile',
            ...
            onSubmit: function (file, ext) {
                if (parseInt($("#news_imgs_content .imgs_items").length) > 4) {
                    top.bootbox.alert('图片最多上传5张！');
                    return false;
                }
                if (!(ext && allowfile.indexOf(ext.toLowerCase()) > -1)) {
                    top.bootbox.alert('错误：无效的文件扩展名！请上传' + allowfile + '格式文件！');
                    return false;
                }
                this.disable();
                intervalTime = window.setInterval(function () {
                    var text = buttonObj.val();
                    if (text.length < 13) {
                        buttonObj.val(text + '.');
                    } else {
                        buttonObj.val('正在上传,请等待!');
                    }
                }, 300);
            },
