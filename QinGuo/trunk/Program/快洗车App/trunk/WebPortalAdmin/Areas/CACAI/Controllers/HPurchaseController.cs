...
namespace WebPortalAdmin.Areas.CACAI.Controllers
{
    ...
    /// 退货单
    /// </summary>
    public class HPurchaseController : BaseController<ModHPurchase>
    {
        string[] title;  //导出的标题
        string[] field;  //导出对应字段
        ...
        public void GetSearch(ref Search search)
        {
            ...
            //退货单号
            if (!string.IsNullOrEmpty(Request["OutNumber"]))
            {
                search.AddCondition("OutNumber like '%" + Request["OutNumber"].ToString() + "%'");
            }
            //采购单号
            ...
            {
                //根据商品名称查询ID
                search.AddCondition("Id in ( select MainId from H_PurchaseDetail where GoodName like '%" + Request["GoodName"].ToString().Trim() + "%')");
            }
            //付款方式
        ...
        public void SearchData()
        {
            ...
            //    }
            //}
            var jsonResult = new BllHPurchase().SearchData(search);
        ...
        public void SearchDataDetail()
        {
            ...
            var jsonResult = new BllHPurchaseDetail().SearchData(search);
            WriteJsonToPage(jsonResult);
        }
        #endregion
        ...
        public void DeleteData(string id)
        {
            ...
            try
            {
                string sql = "update H_Purchase set Status=" + (int)StatusEnum.删除 + " where Id in(" + id + ")";
                if (new BllHPurchase().ExecuteNonQueryByText(sql) > 0)
                {
                    msg.success = true;
                    LogInsert(OperationTypeEnum.操作, "采购退货单删除", "采购退货单删除成功.");
                }
                else
                {
                    msg.success = false;
                    msg.msg = "操作失败";
                    LogInsert(OperationTypeEnum.操作, "采购退货单删除", "采购退货单删除失败.");
                }
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "采购退货单删除", "操作异常信息:" + ex);
            }
        ...
        public void SubmitAudit(string id)
        {
            ...
            try
            {
                ...
                {
                    msg.success = true;
                    LogInsert(OperationTypeEnum.操作, "退货单提交审核", "提交审核成功.");
                }
                else
                {
                    msg.success = false;
                    msg.msg = "操作失败";
                    LogInsert(OperationTypeEnum.操作, "退货单提交审核", "提交审核失败.");
                }
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "退货单提交审核", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        ...
        public void DeleteAll()
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("delete from H_Purchase;");
                sb.AppendLine();
                sb.Append("delete from H_PurchaseDetail;");
                new BllHPurchase().ExecuteNonQueryByText(sb.ToString());
                msg.success = true;
                LogInsert(OperationTypeEnum.操作, "退货单清空", "退货单清空成功.");
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "退货单清空", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        #endregion
        ...
        public void SaveData()
        {
            ...
            try
            {
                ...
                ModHPurchase model = new BllHPurchase().LoadData(id);
                model.Remark = Remark;
                int result = new BllHPurchase().Update(model);
                if (result <= 0)
                {
                    msg.success = false;
                    msg.msg = "修改失败,请稍后再操作!";
        ...
        public void SaveDataDetail()
        {
            ...
            try
            {
                BllHPurchaseDetail Bll = new BllHPurchaseDetail();
                string id = Request["id"].ToString();
                ...
                {
                    ModHPurchaseDetail model = Bll.LoadData(id);
                    ...
                    {
                        //统计盈亏金额
                        string sql = "update H_Purchase set oldPrice=(select SUM(Money) from H_OrderInDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + "),LossPrice=(select SUM(LosstPrice) from H_PurchaseDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + "),TotalPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + ") where Id='" + model.MainId + "';";
                        int retu = Bll.ExecuteNonQueryByText(sql);
                        if (retu > 0)
                        {
                            var OrderIn = new BllHPurchase().LoadData(model.MainId);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public void SaveDetail(ModHPurchaseDetail mod)
        {
            var msg = new ModJsonResult();
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request["modify"])) //修改
                {
                    ModHPurchaseDetail model = bll.LoadData(mod.Id);
                    mod.Id = model.Id;
                ...
                if (msg.success == true)
                {
                    StringBuilder sb = new StringBuilder();
                    //统计盈亏金额
                    sb.Append("update H_Purchase set LossPrice=(select SUM(LosstPrice) from H_PurchaseDetail where MainId='" + mod.MainId + "' and Status!=" + (int)StatusEnum.删除 + "),TotalPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + mod.MainId + "' and Status!=" + (int)StatusEnum.删除 + ") where Id='" + mod.MainId + "';");
                    int retu = bll.ExecuteNonQueryByText(sb.ToString());
                    if (retu > 0)
                    {
                        var OrderIn = new BllHPurchase().LoadData(mod.MainId);
                        msg.success = true;
        ...
        public void DeleteDetail(string id, string key)
        {
            ...
            try
            {
                string sql = "delete from H_PurchaseDetail where Id in(" + id + ")";
                if (new BllHPurchase().ExecuteNonQueryByText(sql) > 0)
                {
                    msg.success = true;
                    LogInsert(OperationTypeEnum.操作, "采购退货单明细删除", "明细删除成功.");
                }
                else
                {
                    msg.success = false;
                    msg.msg = "操作失败";
                    LogInsert(OperationTypeEnum.操作, "采购退货单明细删除", "明细删除失败.");
                }
                if (msg.success == true)
                {
                    StringBuilder sb = new StringBuilder();
                    //统计盈亏金额
                    sb.Append("update H_Purchase set oldPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + key + "' and Status!=" + (int)StatusEnum.删除 + "),LossPrice=(select SUM(LosstPrice) from H_PurchaseDetail where MainId='" + key + "' and Status!=" + (int)StatusEnum.删除 + "),TotalPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + key + "' and Status!=" + (int)StatusEnum.删除 + ") where Id='" + key + "';");
                    int retu = new BllHPurchase().ExecuteNonQueryByText(sb.ToString());
                    if (retu > 0)
                    {
                        var OrderIn = new BllHPurchase().LoadData(key);
                        msg.success = true;
                        msg.msg = OrderIn.LossPrice.ToString() + "," + OrderIn.TotalPrice.ToString();//返回盈亏金额 +供应商金额+合并条数
                    }
                }
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "采购退货单明细删除", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        ...
        public void Combine(string id)
        {
            ...
            try
            {
                BllHPurchaseDetail biz = new BllHPurchaseDetail();
                var model = biz.LoadData(id);
                StringBuilder sb = new StringBuilder();
                if (model != null)
                {
                    //查询与他相同商品信息
                    string sql = "select * from H_PurchaseDetail where StyleNum='" + model.StyleNum + "' and GoodName='" + model.GoodName + "' and FreightNum='" + model.FreightNum + "' and Price='" + model.Price + "' and MainId='" + model.MainId + "' and Id !='" + model.Id + "';";
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            //删除明细
                            sb.Append("delete from H_PurchaseDetail where Id='" + ds.Tables[0].Rows[i]["Id"].ToString() + "';");
                            sb.AppendLine();
                        }
                    }
                    ...
                    {
                        //统计盈亏金额
                        sb.Append("update H_Purchase set oldPrice=(select SUM(Money) from H_PurchaseDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + "),LossPrice=(select SUM(LosstPrice) from H_PurchaseDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + "),TotalPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + model.MainId + "' and Status!=" + (int)StatusEnum.删除 + ") where Id='" + model.MainId + "';");
        ...
        public void CombineAll(string id)
        {
            ...
            try
            {
                ...
                //查询与他相同商品信息
                string sql = "select IsEdit=0,* from H_PurchaseDetail where MainId ='" + id + "' and Status!=" + (int)StatusEnum.删除 + ";";
                DataSet ds = biz.ExecuteDataSet(sql);
                StringBuilder sb = new StringBuilder();
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        if (Convert.ToInt32(ds.Tables[0].Rows[i]["IsEdit"].ToString()) == 0)
                        {
                            ...
                            for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                            {
                                if (ds.Tables[0].Rows[j]["Id"].ToString() != Id && Convert.ToInt32(ds.Tables[0].Rows[j]["IsEdit"].ToString()) == 0)
                                {
                                    if (ds.Tables[0].Rows[j]["StyleNum"].ToString() == ds.Tables[0].Rows[i]["StyleNum"].ToString() && ds.Tables[0].Rows[j]["FreightNum"].ToString() == ds.Tables[0].Rows[i]["FreightNum"].ToString() && Convert.ToDecimal(ds.Tables[0].Rows[j]["Price"].ToString()) == Convert.ToDecimal(ds.Tables[0].Rows[i]["Price"].ToString()))
                                    {
                                        ...
                                        //删除明细
                                        sb.Append("update H_PurchaseDetail  set Status=" + (int)StatusEnum.删除 + " where Id='" + ds.Tables[0].Rows[j]["Id"].ToString() + "';");
                                        sb.AppendLine();
                                    }
                                }
                            }
                            if (Combin == true)
                            {
                                sb.Append(@"update H_PurchaseDetail set Code='" + Code + "',GoodUnit='" + GoodUnit + "',GoodName='" + GoodName + "',Remark='" + Remark + "',Count=" + Count + ",BillNum=" + Count + ",BillPrice=" + Price + ",Money=" + Money + ",BillMoney=" + Money + ",CheckNum=0,CheckPrice=0,LosstPrice=0 where Id='" + ds.Tables[0].Rows[i]["Id"].ToString() + "';");
                                sb.AppendLine();
                            }
                        }
                    }
                    //统计盈亏金额
                    sb.Append("update H_Purchase set oldPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + id + "' and Status!=" + (int)StatusEnum.删除 + "),LossPrice=(select SUM(LosstPrice) from H_PurchaseDetail where MainId='" + id + "' and Status!=" + (int)StatusEnum.删除 + "),TotalPrice=(select SUM(BillMoney) from H_PurchaseDetail where MainId='" + id + "' and Status!=" + (int)StatusEnum.删除 + ") where Id='" + id + "';");
                    sb.AppendLine();
                    int retu = biz.ExecuteNonQueryByText(sb.ToString());
                    if (retu > 0)
                    {
                        var OrderIn = new BllHPurchase().LoadData(id);
                        msg.success = true;
            ...
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "退货单明细合并", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        ...
        public ActionResult ImportOut()
        {
            ...
            try
            {
                ...
                LogInsert(OperationTypeEnum.操作, "采购退货单批量导出", "导出成功.");
            }
            catch (Exception ex)
            {
                LogInsert(OperationTypeEnum.异常, "采购退货单批量导出", "操作异常信息:" + ex);
        ...
        //    doc.FileName = DateTime.Now.ToString("yyyyMMdd") + "采购退货单批量导出.xls";
        ...
        public void EditDetailRemark()
        {
            ...
            try
            {
                ...
                if (result <= 0)
                {
                    msg.success = false;
                    msg.msg = "修改失败,请稍后再操作!";
                    LogInsert(OperationTypeEnum.操作, "退货单备注批量修改", "备注修改失败.");
                }
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "退货单备注批量修改", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        #endregion
        ...
        public void CancelAudit(string id)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("update H_Purchase set FinancialState=-2,FinancialDT=getdate() where Id in(" + id + ")");
                sb.AppendLine();
                if (new BllHPurchase().ExecuteNonQueryByText(sb.ToString()) > 0)
                {
                    msg.success = true;
                    LogInsert(OperationTypeEnum.操作, "退货单驳回审核", "驳回审核成功.");
                }
                else
                {
                    msg.success = false;
                    msg.msg = "操作失败";
                    LogInsert(OperationTypeEnum.操作, "退货单驳回审核", "驳回审核失败.");
                }
            }
            catch (Exception ex)
            {
                msg.msg = "操作失败：" + ex.Message;
                LogInsert(OperationTypeEnum.异常, "退货单驳回审核", "操作异常信息:" + ex);
            }
            WriteJsonToPage(msg.ToString());
        }
        ...
        public void SubmitData(string id)
        {
            ...
            try
            {
                BllHPurchase Biz = new BllHPurchase();
                ...
                for (int i = 0; i < str.Length; i++)
                {
                    ...
                    if (model != null)
                    {
                        ...
                        sb.Append("'" + newid + "',");
                        sb.Append("" + str[i] + ",");
                        sb.Append("1,");//扣减状态 1:未完成 2：部分完成 3：已完成
                        sb.Append("'',");
                        sb.Append("" + model.TotalPrice + ",");//Price
                        sb.Append("0,");
                        sb.Append("0,");
                        sb.Append("'',");
                        sb.Append("''");
                        ...
                        sb.AppendLine();
                        //更改主单据提交状态
                        sb.Append("update H_Purchase set FinancialState=2 where Id='" + model.Id + "';");
                        sb.AppendLine();
                        #endregion
                        ...
                        ////查找财务池中是否有相同供应商的入库单信息
                        //string sql = "select top 1 * from view_HOrderInRelation(nolock) where CusterId='" + CusterId + "' and PoolStatus!=2 order by FinancialDT asc;";
                        //DataSet ds = Biz.ExecuteDataSet(sql);
                        //if (ds.Tables[0].Rows.Count > 0)
                        //{
                        //    //存在供应商
                        //    var TotalPrice = Convert.ToDecimal(ds.Tables[0].Rows[0]["TotalPrice"].ToString());//供应商金额
                        //    //当前提交单据的退货金额
                        //    var BackPrice = model.TotalPrice;
                        //    //获取任务池中明细金额
                        //    sql = "select * from view_HPurchaseRelation(nolock) where CusterId='" + CusterId + "'  and PoolStatus!=3  order by FinancialDT asc;";
                        ...
                        //        }
                        //        //判断当前单据金额是否满足主表金额
                        //        if ((decimalCount + BackPrice) < TotalPrice)
                        //        {
                        //            #region===添加单据到任务池
                        //            string newid = Guid.NewGuid().ToString();
                        ...
                        //            #endregion
                        //        }
                        //        else if ((decimalCount + BackPrice) == TotalPrice)
                        //        {
                        //            #region===直接提交单据到付款单
                        //            //1：设置任务池为完成状态
                        //            string newid = Guid.NewGuid().ToString();
                        ...
                        //            sb.AppendLine();
                        //            //2：更改主单据状态为完成状态
                        //            sb.Append(@"update H_OrderInRelation set Status=2 where Id=" + ds.Tables[0].Rows[0]["RelationId"].ToString() + ";");
                        //            sb.AppendLine();
                        //            //3：添加主单据到付款单
                        ...
                        //            sb.Append("'',");
                        //            sb.Append("'" + newid + "',");
                        //            sb.Append("'" + ds.Tables[0].Rows[0]["Id"].ToString() + "',");//入库单主键
                        ...
                        //                //6:更改以前的扣减状态 1:未完成 2：部分完成 3：已完成
                        ...
                        //            #region===进项拆分价格
                        //            //进项拆分价格
                        //            var needPrice = (decimalCount + BackPrice) - TotalPrice;//主表需要拆分的金额 ((100+50)-50)
                        //            string newid = Guid.NewGuid().ToString();
                        //            //1:把拆分的留在池子
                        //            sb.Append(@"insert into H_PurchaseRelation(Id,PurchaseId,Status,Remark,Price,IsDecompose,DecomposePrice,OutNumberS)values(");
                        //            sb.Append("'" + newid + "',");
                        ...
                        //            sb.Append("0,");//PayStatus
                        //            sb.Append("'',");
                        //            sb.Append("'" + ds.Tables[0].Rows[0]["RelationId"].ToString() + "',");
                        //            sb.Append("'" + ds.Tables[0].Rows[0]["Id"].ToString() + "',");//入库单主键
                        //            sb.Append("" + (CusterModel.CheckoutType == null ? 0 : CusterModel.CheckoutType) + ",");
                        //            sb.Append("" + (CusterModel.PaymentType == null ? 0 : CusterModel.PaymentType) + ",");
                        //            sb.Append("'" + CusterModel.AccountName + "',");
                        ...
                        //                //7:更改以前的扣减状态 1:未完成 2：部分完成 3：已完成
                        //                sb.Append("update H_PurchaseRelation set Status=3 where Id='" + Relation.Tables[0].Rows[q]["RelationId"] + "';");
                        //                sb.AppendLine();
                        //            }
                        //            #endregion
                        ...
                        //            sb.AppendLine();
                        //            //2：更改主单据状态为完成状态
                        //            sb.Append("update H_Purchase set FinancialState=2,GetNumber='" + ds.Tables[0].Rows[0]["InNumber"].ToString() + "' where Id=" + str[i] + ";");
                        //            sb.AppendLine();
                        //            //3：添加主单据到付款单
                        //            string FinanceId = Guid.NewGuid().ToString();
                        //            sb.Append(@"insert into H_Finance(Id,CreateTime,CreaterId,Status,ReturnAmount,ProfitTotal,PaymentAmmount,PayStatus,FinanceRemark,RelationId,OrderInId,CheckoutType,PaymentType,AccountName,AccountNum) values(");
                        ...
                        //        {
                        //            //进项拆分价格
                        //            var needPrice = BackPrice - TotalPrice;//主表需要拆分的金额 (150-50)
                        //            string newid = Guid.NewGuid().ToString();
                        //            //1:把拆分的留在池子
                        //            sb.Append(@"insert into H_PurchaseRelation(Id,PurchaseId,Status,Remark,Price,IsDecompose,DecomposePrice,OutNumberS)values(");
                        //            sb.Append("'" + newid + "',");
