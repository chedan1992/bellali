...
function gridrowclick(grid, rowindex, e) {
    ...
    if (grid.store.data.length > 0) {
        ...
        if (rows.length == 1) {
            ...
            SourceGrid.getStore().proxy.conn.url = '/CACAI/InComeBill/SearchDataDetail?MainId=' + treeNodeId;
    ...
    if (rows.length > 0) {
        var confirm = top.Ext.MessageBox.confirm('系统确认', '您确定要导出当前选中的信息吗?', function (e) {
            if (e == "yes") {
                ...
                Ext.Ajax.request({
                    url: '/CACAI/InComeBill/ImportOut',
                    params: {
                        IdList: ids.join(",")
                    },
                    methond: 'post',
...
//                var url = '/CACAI/InComeBill/ImportOut?IdList=' + ids.join(",") + '&date=' + new Date();
...
function UoLoad(Id) {
   var grid = Ext.getCmp("gg");
    ...
    if (rows.length == 1) {
        top.Ext.MessageBox.wait('', '系统正在执行中,请稍后...'); //显示等待信息框   
        Ext.Ajax.request({
            url: "/SysFileAttach/FileUpload?KeyId=" + rows[0].get("Id") + "&ModelCode=InCome",
            isUpload: true,
            form: "upForm" + Id,
...
function BtnNewAdd() {
    ...
    if (rows.length == 1) {
        ...
        url = '/CACAI/InComeBill/SaveDetail?MainId=' + Id;
...
function BtnNewEdit() {
    ...
    if (rows.length == 1) {
        ...
        win.show();
        url = '/CACAI/InComeBill/SaveDetail?Id=' + key + '&modify=1';
    }
    else {
        MessageInfo("请选中一条记录！", "statusing");
    }
    ...
    if (rows.length > 0) {
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要删除该信息吗?', function (e) {
            if (e == "yes") {
                ...
                Ext.Ajax.request({
                    url: '/CACAI/InComeBill/DeleteDetail',
...
function BtnCombine() {
    if (Ext.getCmp('rightGrid').getStore().totalLength > 0) {
        ...
        if (rows.length == 1) {
            ...
            top.Ext.MessageBox.wait('', '系统正在执行中,请稍后...'); //显示等待信息框   
            Ext.Ajax.request({
                url: '/CACAI/InComeBill/CombineAll',
                params: { id: Id },
                success: function (response) {
                    var rs = eval('(' + response.responseText + ')');
                    if (rs.success) {
...
function GTP_AllEdit() {
    ...
    if (rows.length >= 1) {
        ...
        var form = top.Ext.getCmp('formPanel');
        form.form.reset();
        url = '/CACAI/InComeBill/EditRemark?ids=' + ids.join(',');
        window.show();
    }
    else {
        MessageInfo("请选中一条记录！", "statusing");
...
function BtnEditDetail() {
    ...
    if (rows.length == 1) {
        ...
        form.form.loadRecord(rows[0]);
        win.show();
        url = '/CACAI/InComeBill/SaveDataDetail?Id=' + key + '&modify=1';
...
function EditDate() {
    ...
    if (rows.length == 1) {
        ...
        form.form.loadRecord(rows[0]);
        win.show();
        url = '/CACAI/InComeBill/SaveData?Id=' + key + '&modify=1';
    }
    else {
        MessageInfo("请选中一条记录！", "statusing");
    }
...
function DeleteDate() {
    ...
    if (rows.length > 0) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            var InNumber = rows[i].get("InNumber"); //入库单号
            if (FinancialState == 1 || FinancialState == -1) {
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】已提交，不能进行删除。", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                return false;
            }
            else if (FinancialState == 2) {
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】已审核通过，不能进行删除。", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                return false;
            }
            else if (FinancialState == 3) {
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】已支付，不能进行删除。", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                return false;
            }
            ids.push("'" + rows[i].data["Id"] + "'");
        }
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要删除该信息吗?', function (e) {
            if (e == "yes") {
                top.Ext.MessageBox.wait('', '系统正在执行中,请稍后...'); //显示等待信息框   
                Ext.Ajax.request({
                    url: '/CACAI/InComeBill/DeleteData',
                    params: { id: ids.join(",") },
                    success: function (response) {
                        var rs = eval('(' + response.responseText + ')');
                        if (rs.success) {
...
function GTPsubmit() {
    ...
    if (rows.length >= 1) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (HasFileAttach != 1) {
                flag = false;
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】凭证不能为空，请先上传凭证!", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                break;
            }
            if (FinancialState == 1) {
                flag = false;
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】单据已提交，不能重复提交!", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                break;
            }
            else if (FinancialState == 2) {
                flag = false;
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】单据已审核，不能重复提交!", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                break;
            }
            else if (FinancialState == 3) {
                flag = false;
                top.Ext.Msg.show({ title: "信息提示", msg: "入库单号【" + InNumber + "】单据已支付，不能重复提交!", buttons: Ext.Msg.OK, icon: Ext.MessageBox.INFO });
                break;
            } 
        }
        if (flag == true) {
            var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要提交选中的记录信息吗?', function (e) {
                if (e == "yes") {
                    ...
                    top.Ext.MessageBox.wait('', '系统正在执行中,请稍后...'); //显示等待信息框   
                    Ext.Ajax.request({
                        url: '/CACAI/InComeBill/SubmitAudit',
                        params: { id: ids.join(",") },
                        success: function (response) {
                            var rs = eval('(' + response.responseText + ')');
                            if (rs.success) {
    ...
    if (rows.length == 1) {
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要驳回选中的记录信息吗?', function (e) {
            if (e == "yes") {
                ...
                top.Ext.MessageBox.wait('', '系统正在执行中,请稍后...'); //显示等待信息框   
                Ext.Ajax.request({
                    url: '/CACAI/InComeBill/CancelAudit',
                    params: { id: ids.join(",") },
                    success: function (response) {
                        var rs = eval('(' + response.responseText + ')');
                        if (rs.success) {
...
function afterEdit(e) {
    ...
    if (selectNode!=null) {
        ...
        if (FinancialState == -1 || FinancialState == 1 || FinancialState == 2 || FinancialState == 3) {
            ...
            Ext.Ajax.request({
                url: '/CACAI/InComeBill/SaveDataDetail?modify=1',
                params: {
                    id: record.Id,
                    BillNum: BillNum,
                    BillPrice: BillPrice,
