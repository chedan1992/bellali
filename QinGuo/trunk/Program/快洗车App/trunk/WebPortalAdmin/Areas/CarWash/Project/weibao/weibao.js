...
function EditDate() {
    ...
    if (rows.length == 1) {
        ...
        url = '/SysGroup/SaveData?Id=' + key + '&modify=1&Type=' +2;
        window.show();
...
function EnableUse() {
    ...
    if (rows.length == 1) {
        ...
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要启用该维保项目吗?', function (e) {
            if (e == "yes") {
                var ids = [];
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].data["Id"]);
...
function DisableUse() {
    ...
    if (rows.length == 1) {
        ...
        var confirm = top.Ext.MessageBox.confirm('系统确认', '禁用后,该维保项目将不再使用吗?', function (e) {
            if (e == "yes") {
                var ids = [];
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].data["Id"]);
