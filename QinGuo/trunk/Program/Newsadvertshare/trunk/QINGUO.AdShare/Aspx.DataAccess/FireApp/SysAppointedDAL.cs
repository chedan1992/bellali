...
namespace QINGUO.DAL
{
    public class SysAppointedDAL : BaseDAL<ModSysAppointed>, ISysAppointed
    {
        ...
        public DataSet Total(string sysId)
        {
            ...
            {
                sb.Append("Select (select COUNT(Id) from Sys_Master where IsMain =1 and (Status!=" + (int)StatusEnum.删除 + " and Status!=-2) and  Cid='" + sysId + "' and (Attribute>=" + (int)AdminTypeEnum.单位用户 + " and Attribute<=" + (int)AdminTypeEnum.供应商用户 + ")) as UserCount, ");
            ...
            {
                sb.Append("Select (select COUNT(Id) from Sys_Master where IsMain =1 and (Status!=" + (int)StatusEnum.删除 + " and Status!=-2) and (Attribute>=" + (int)AdminTypeEnum.单位用户 + " and Attribute<=" + (int)AdminTypeEnum.供应商用户 + ")) as UserCount, ");
