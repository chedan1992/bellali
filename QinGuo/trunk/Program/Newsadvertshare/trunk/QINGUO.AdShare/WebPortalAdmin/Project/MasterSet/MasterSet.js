...
function changePanel(value) {
    if (Ext.getCmp("AutomaticSave").getValue() == true) {
...
function treeitemclick(node, e) {
    ...
    Ext.getCmp("comModel").store.proxy = new Ext.data.HttpProxy({ url: '/DeptSet/GetTreeByMaster?OrganizaId=' + treeNodeId, method: 'POST' });
    Ext.getCmp("comModel").store.reload();
   // ChangeModel(treeNodeId);

    Ext.getCmp("comModel").setValue("");
    Ext.getCmp("comModel").setRawValue("");
    Ext.getCmp("AutomaticSave").setValue(false);
}
...
function ChangeModel(key) {
    ...
    respon.open("post", "/DeptSet/GetData?id=" + key + "&Cid=" + treeNodeId + "&SetType=2", false); //配置类型(1:岗位配置 2:特殊人员配置)
    respon.send(null);
    ...
    if (response != null) {
        ...
        if (response.DeptSetId == "" || response.DeptSetId == null) {
            ...
            Ext.getCmp("AutomaticSave").setValue(true);
...
function SaveDate() {
    ...
    if (Ext.getCmp("AutomaticSave").getValue() == false && saveSign==false) {
        MessageInfo("您还未开启考勤,无法保存！", "error");
        return false;
    }
    if (treeNodeId == -1) {
        MessageInfo("请先选择岗位！", "error");
        return false;
    }
    else {
        if (formPanel.getForm().isValid()) {//如果验证通过
            ...
            //是否开启考勤
            var IsSave = Ext.getCmp("AutomaticSave").getValue();
            var para = { DeptId: treeNodeId, MasterId: comModel, Limit: LimitNum, SignType: SignTypeNum, SetType: 2, Detail: Strategy, CanSave: IsSave };
            formPanel.getForm().submit({
                ...
                success: function (form, action) {
                    ...
                    if (flag.success) {
                        ...
                        ChangeModel(flag.msg);//传递MasterId
                    } else {
