layui.config({
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
        ...
        if (options.url) { //Ajax请求
            ...
            var url = options.url.indexOf('&') != -1 ? options.url + '&_token=' + Comm.db('_token') : options.url + '?_token=' + Comm.db('_token');
    ...
    Class.prototype.eachCols = function (callback) {
        ...
        layui.each(cols, function (i1, item1) {
            for (var k in item1) {
                ...
                if (item2 && item2.PARENT_COL) return; //如果是子列，则不进行追加，因为已经存储在父列中
                if (item2)
                    arrs.push(item2)
            }
        });
    ...
    Class.prototype.renderTd = function (obj, cols, numbers, i3) {
        ...
        var td = ['<td data-field="' + field + '" ' + function () {
            ...
            }() + '">' + treeImgHtml + '<p style="width: auto;height: 100%;height:28px;line-height:28px;">' + function () {
