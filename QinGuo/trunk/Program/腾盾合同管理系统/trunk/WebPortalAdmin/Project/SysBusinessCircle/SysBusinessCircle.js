...
function treeitemclick(node, e) {
    if (node.isLeaf()) {
        treeNodeId = node.attributes.Code;
...
function loadRefrush() {
    ...
    Ext.getCmp("gg").getStore().proxy.conn.url = '/SysBusinessCircle/SearchData?ProvinceCode=' + node.attributes.Code;
...
function EnableUse() {
    ...
    if (rows.length == 1) {
        if (rows[0].data["Status"] == 1) {
            MessageInfo("该地区已经启用！", "statusing");
            return;
        }
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要启用该地区吗?', function (e) {
            if (e == "yes") {
                var ids = [];
...
function DisableUse() {
    ...
    if (rows.length == 1) {
        if (rows[0].data["Status"] == 0) {
            MessageInfo("该地区已经禁用！", "statusing");
            return;
        }
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要禁用该地区吗?', function (e) {
            if (e == "yes") {
                var ids = [];
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].data["Id"]);
...
function AddDate() {
    ...
    if (node == null || node.id=='root') {
        MessageInfo("请先选择所在城市！", "statusing");
        return;
    }
...
function SaveDate() {
    ...
    if (formPanel.getForm().isValid()) {//如果验证通过
        url += "&BParentID=0&CityID=" + Ext.getCmp("tree").getSelectionModel().getSelectedNode().id;
...
function CreateFromWindow(title) {
    var form = new top.Ext.FormPanel({
        ...
        items: [
                {
                    name: 'BName',
                    fieldLabel: '<span class="required">*</span>区域名称',
                    emptyText: '输入区域的名称', ////textfield自己的属性
                    allowBlank: false,
                    maxLength: 20,
                    maxLengthText: '区域名称长度不能超过20个字符'
                }
        ]
    });
