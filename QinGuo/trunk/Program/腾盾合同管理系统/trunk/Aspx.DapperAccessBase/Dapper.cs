...
namespace Dapper
{
    public class Database : IDisposable
    {
        ...
        public Database(string connectionStringName)
        {
            ...
            string sConnection = ConfigurationManager.ConnectionStrings[connectionStringName].ConnectionString;
            _connectionString = sConnection;// BEGDC.Utility.Security.EncryptTool.Decrypt(sConnection, BEGDC.Utility.Configuration.ConfigConst.Key);
            _providerName = providerName;
            CommonConstruct();
        }
        ...
        /// <param name="poco">The POCO object whose primary key value will be used to delete the row</param>
        /// <returns>The number of rows affected</returns>
        public int DeleteStatus(string tableName, string primaryKeyName, object poco)
        {
            return DeleteStatus(tableName, primaryKeyName, poco, null);
        }

        public int DeleteStatus(object poco)
        {
            var pd = PocoData.ForType(poco.GetType());
            return DeleteStatus(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, poco);
        }
        ...
        /// <param name="primaryKeyValue">The value of the primary key identifing the record to be deleted (or null, or get this value from the POCO instance)</param>
        /// <returns>The number of rows affected</returns>
        public int DeleteStatus(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            // If primary key value not specified, pick it up from the object
            if (primaryKeyValue == null)
            {
            ...
            // Do it
            var sql = string.Format("update {0} set Status=-1 WHERE {1}=@0", _dbType.EscapeTableName(tableName), _dbType.EscapeSqlIdentifier(primaryKeyName));
            return Execute(sql, primaryKeyValue);
        }
        #endregion
