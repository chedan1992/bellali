Ext.onReady(function () {
    ...
    var ChartColumn = function () {
        ...
        var myChart = echarts.init(document.getElementById('chartPart'));
        // 指定图表的配置项和数据
        var option = {
            title: {
                text: new Date().getFullYear() + '年',
                subtext: '所有单位设备月份巡检统计(月份)'
            },
            tooltip: {
                trigger: 'axis'
            },
    ...
    var CharPart = function () {
        ...
        function generateData() {
            //请求图表统计
            var respon = Ext.lib.Ajax.getConnectionObject().conn;
            respon.open("post", "/SysAppointed/ChartPart", false); //获取页面按钮
            respon.send(null);
            var result = Ext.util.JSON.decode(respon.responseText);
            if (result.success) {
                for (var i = 0; i < result.data.length; ++i) {
                    Jsondata.push({ value: result.data[i].value, name: result.data[i].name });
