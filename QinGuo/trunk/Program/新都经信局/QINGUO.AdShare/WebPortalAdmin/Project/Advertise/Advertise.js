...
function formartImg(val, rows) {
    return '<img src="' + val + '" style="max-width:250px;max-height:100px;"/>';
}
        ...
        var comName = new Ext.form.ComboBox({
            ...
            listeners: {
                select: function (mysel) {
                    ...
                    else if (itemstr.indexOf('幻灯片位置') > -1) {
                        Ext.getCmp("comTimeContent").hide();
                        Ext.getCmp("comContent").hide();
                        Ext.getCmp("comEnum").hide();
                    ...
                    else if (itemstr.indexOf('幻灯片类型') > -1 && itemstr.length == 4) {
                        Ext.getCmp("comTimeContent").hide();
                        Ext.getCmp("comContent").hide();
                        Ext.getCmp("comEnum").hide();
                        Ext.getCmp("Attribute").hide();
...
function LookInfo() {
    ...
    if (rows.length == 1) {
        ...
        if (parent) {
            ...
            parent.AddTabPanel(title, key, tabId, '/Advertise/AdvertiseView?Id=' + key);
        }  
    }
    else {
...
function EnableUse() {
    ...
    if (rows.length == 1) {
        if (rows[0].data["Status"] == 1) {
            MessageInfo("该幻灯片已经启用！", "statusing");
            return;
        }
        if (rows[0].data["Status"] ==-2) {
            MessageInfo("该幻灯片已过期,不能启用！", "statusing");
            return;
        }
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要启用该幻灯片吗?', function (e) {
            if (e == "yes") {
                var ids = [];
...
function DisableUse() {
    ...
    if (rows.length == 1) {
        if (rows[0].data["Status"] == 0) {
            MessageInfo("该幻灯片已经禁用！", "statusing");
            return;
        }
        if (rows[0].data["Status"] == -2) {
            MessageInfo("该幻灯片已过期,不能禁用！", "statusing");
            return;
        }
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要禁用该幻灯片吗?', function (e) {
            if (e == "yes") {
                var ids = [];
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].data["Id"]);
...
function AddDate() {
    if (parent) {
        var tabId = parent.GetActiveTabId(); //公共变量
        parent.AddTabPanel('新增幻灯片', 'AdvertiseWindow',tabId, '/Advertise/AdvertiseEdit');
    }
}
...
function EditDate() {
    ...
    if (rows.length == 1) {
        ...
        if (parent) {
            var tabId = parent.GetActiveTabId(); //公共变量
            parent.AddTabPanel(title, 'ShopAdvertiseWindow',tabId, '/Advertise/AdvertiseEdit?Id=' + key + '&modify=1');
        }  
    }
    else {
        MessageInfo("请选中一条记录！", "statusing");
...
function DeleteDate() {
    ...
    var rows = grid.getSelectionModel().getSelections();
    if (rows.length == 1) {
        var confirm = top.Ext.MessageBox.confirm('系统确认', '确认要删除该幻灯片吗?', function (e) {
            if (e == "yes") {
                var ids = [];
                for (var i = 0; i < rows.length; i++) {
                    ids.push(""+rows[i].data["Id"]+"");
