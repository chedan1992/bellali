Ext.onReady(function () {
    ...
    //数据源
    var store = GridStore(EDynamic, '/EDynamic/SearchData', className, "IsTop desc,Sort ASC,CreateTime", "desc");
    ...
    grid.store.on("load", function () {
        var dr=grid.getSelectionModel().selectFirstRow(); //用这个就可以了，前提是你要有SelectionModel的实现
        var rows = grid.getSelectionModel().getSelections();
        if (rows.length >= 1) {
            document.getElementById("mbid").setAttribute("src", "/EDynamic/DocumentView?Id=" + rows[0].get("Id"));
        }
    });
    ...
    var viewport = new Ext.Viewport({
        ...
        items: [
            ...
            collapseMode: 'mini', //出现小箭头
            deferredRender: false,
            html: "<iframe id='mbid' src='' style='width:100%; height:100%;border:0px;'></iframe>"
        }]
    });
});