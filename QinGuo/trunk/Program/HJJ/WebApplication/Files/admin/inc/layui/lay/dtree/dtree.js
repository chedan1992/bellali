...
layui.define(['jquery','layer','element','form'], function(exports) {
	...
	var LI_NAV_ROOT = "nav-tree-item-index", LI_NAV_CHILD = "nav-tree-ul-sid", LI_NAV_ITEM = "nav-tree-item", 
		...
		LI_DIV_TOOLBAR = "nav-tree-toolbar-div", TOOLBAR_SHOW = "nav-tree-toolbar-div-show", LI_DIV_TOOLBAR_ADD = "icon-jia1", LI_DIV_TOOLBAR_EDIT = "icon-bianji", LI_DIV_TOOLBAR_DEL = "icon-jian1",
	...
	var eventName = {
		addToolbar: "addToolbar",						//点击节点图标显示新增框
		editToolbar: "editToolbar",						//点击节点图标显示编辑框
		delToolbar: "delToolbar",						//点击节点图标删除
	...
	DTree.prototype.init = function(){
		...
		if(_this.data && _this.data.length > 0) {
			...
			// 复选框半选状态设置
			if(_this.checkbar && _this.checkbarType=='no-all'){
	...
	DTree.prototype.loadListTree = function(pListData, listData, level, $ul){		
		...
		if (pListData.length > 0){
			for (var i = 0; i < pListData.length; i++) {
				...
				var childListData = _this.queryListTreeByPid(id, listData);
				...
				if ($ul.hasClass(UL_ROOT)) {
					...
					$ul.append(_this.getLiItemDom(dom, id, parentId, isLast, level, basicData, "root"));
				} else {
					var dom = _this.getDom(id, parentId, title, isLast, iconClass, checkArr, level);
					$ul.append(_this.getLiItemDom(dom, id, parentId, isLast, level, basicData, "item"));
				}
	...
	// 获取拼接好的li
	DTree.prototype.getLiItemDom =  function(dom, id, parentId, isLast, level, basicData, flag) {	
		var _this = this,
			rootId = _this.elem;
		var div = "<div class='"+LI_DIV_ITEM+"' data-id='"+id+"' root-id='"+rootId+"' d-click='"+eventName.itemNodeClick+"' data-basic='"+basicData+"' d-contextmenu='false'>";
		if(_this.toolbar){
			if(_this.toolbarLoad == "node") {
				div = "<div class='"+LI_DIV_ITEM+"' data-id='"+id+"' root-id='"+rootId+"' d-click='"+eventName.itemNodeClick+"' data-basic='"+basicData+"' d-contextmenu='true'>";
			} 
	...
	DTree.prototype.getToolbarDom = function(){	
		...
		if(toolbarShow.length > 0){
			for(var i=0; i<toolbarShow.length; i++){
				...
				if(show == "add"){
					dd += "<dd><a d-click='"+eventName.addToolbar+"'><i class='"+ICONFONT+" "+LI_DIV_TOOLBAR_ADD+"'></i>新增节点</a></dd>";
				}
				if(show == "edit"){
					dd += "<dd><a d-click='"+eventName.editToolbar+"'><i class='"+ICONFONT+" "+LI_DIV_TOOLBAR_EDIT+"'></i>编辑节点</a></dd>";
				}
				if(show == "delete"){
					dd += "<dd><a d-click='"+eventName.delToolbar+"'><i class='"+ICONFONT+" "+LI_DIV_TOOLBAR_DEL+"'></i>删除节点</a></dd>";
				}
			}
	...
	DTree.prototype.bindBrowserEvent = function(nodes){
		...
		_this.obj.on("click", "i[data-spared]", function(event) {
			...
			$toolBarDiv.find(".layui-nav-child").removeClass(TOOLBAR_SHOW);
		...
		_this.obj.on("dblclick", "div[d-click='"+eventName.itemNodeClick+"']", function(event) {
			...
			var $div = $(this),
				...
				$p_ul = $p_li.parent("ul");
			var $toolBarDiv = _this.obj.prev('div#dtree_toolbar_'+_this.obj[0].id);
			$toolBarDiv.find(".layui-nav-child").removeClass('layui-anim-upbit layui-show');
		...
		_this.obj.on("contextmenu", "div[d-click='"+eventName.itemNodeClick+"'][d-contextmenu]", function(e){
			...
			if(_this.toolbar){
				...
				mx = e.pageX +5,
				my = $div.offset().top - $(_this.toolbarScroll).offset().top +10;
				
				if(contextmenu == "true"){
					_this.obj.find("div[data-id]").parent().find("."+NAV_THIS).removeClass(NAV_THIS);
					$div.addClass(NAV_THIS);
					$toolBarDiv.find(".layui-nav-child").addClass('layui-anim-upbit layui-show');
		...
		$(_this.toolbarScroll).scroll(function() {
			var $toolBarDiv = _this.obj.prev('div#dtree_toolbar_'+_this.obj[0].id);
			$toolBarDiv.find(".layui-nav-child").removeClass('layui-anim-upbit layui-show');
		});
				...
				success: function(layero, index){
					form.on("submit(dtree_addNode_form)",function(data){
						...
						var parentId = $div.attr("data-id"),
							id = $div.attr("data-id")+"_node_"+_this.addIndex,
							isLeaf = true,
							isChecked = "0",
			    		...
			    		if(returnID){
			    			...
			    			if(typeof returnID === "object"){
						    	...
						    	$ul.append(_this.getLiItemDom(dom, returnID.id, returnID.parentId, returnID.isLast, returnID.level, basicData, "item"));
		...
		_this.obj.prev('div#dtree_toolbar_'+_this.obj[0].id).on("click", "a[d-click='"+eventName.editToolbar+"']", function(event) {
			...
			layer.open({
				...
				content: content,
				success: function(layero, index){
					var param = _this.toolbarFun.editTreeLoad(_this.getRequestParam(node));
					console.log(param);
