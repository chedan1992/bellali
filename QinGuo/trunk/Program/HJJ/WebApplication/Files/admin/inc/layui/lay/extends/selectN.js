...
layui.define(['jquery', 'form'], function (exports) {
    ...
    var obj = function (config) {
        ...
        this.setfilter = function () {
            var o = this, c = o.config;
            if (Object.prototype.toString.call(c.filter) != '[object Array]') {
                return c.filter;
            }
            else {
                var i = $(c.elem).find('select').length;
                return c.filter.hasOwnProperty(i) ? c.filter[i] : '';
            }
        }
        ...
        this.createSelect = function (optionData) {
            ...
            html += ' <select ' + o.setSearch() + 'lay-filter="' + o.setfilter() + '" name="'+o.setfilter()+'">';
    ...
    obj.prototype.render = function () {
        ...
        if (c.filter != "" && c.filter && c.filter.length > 0) {
            for (var i = 0; i < c.filter.length; i++) {
                //监听下拉事件
                form.on('select(' + c.filter[i] + ')', function (data) {
                    o.change($(data.elem));
                });
            }
            ...
            c.filter = c.elem.replace('#', '').replace('.', '');
            //监听下拉事件
