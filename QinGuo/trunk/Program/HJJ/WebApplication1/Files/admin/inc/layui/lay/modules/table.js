...
layui.define(["laytpl", "laypage", "layer", "form", "util"],
    function (e) {
        ...
        F.prototype.config = {
            ...
            //2018-11-29 zj 修改
            F.prototype.initCheckboxchecked = function (t) {
                var i = this, a = i.config;
                if (a.isremember) {
                    var ti = i;
                    d.cache.temp = d.cache.temp == undefined || d.cache.temp == null ? [] : d.cache.temp;
                    a.tempid = a.tempid == undefined || a.tempid == null || a.tempid == "" ? 'id' : a.tempid;

                    //在缓存中找到id ,然后设置data表格中的选中状态
                    //循环所有数据，找出对应关系，设置checkbox选中状态
                    for (var k = 0; k < t.data.length; k++) {
                        for (var j = 0; j < d.cache.temp.length; j++) {
                            //数据id和要勾选的id相同时checkbox选中
                            if (t.data[k][a.tempid] == d.cache.temp[j][a.tempid]) {
                                //这里才是真正的有效勾选
                                t.data[k]["LAY_CHECKED"] = 'true';
                                //找到对应数据改变勾选样式，呈现出选中效果
                                var index = t.data[k]['LAY_TABLE_INDEX'];
                                $(ti.elem).find('tr[data-index=' + index + '] input[type="checkbox"]').prop('checked', true);
                                $(ti.elem).find('tr[data-index=' + index + '] input[type="checkbox"]').next().addClass('layui-form-checked');
                            }
                        }
                    }
                    //设置全选checkbox的选中状态，只有改变LAY_CHECKED的值， d.checkStatus才能抓取到选中的状态
                    var checkStatus = d.checkStatus(i.key);
                    if (checkStatus.isAll) {
                        $(ti.elem).find('.layui-table-header th[data-field="0"] input[type="checkbox"]').prop('checked', true);
                        $(ti.elem).find('.layui-table-header th[data-field="0"] input[type="checkbox"]').next().addClass('layui-form-checked');
                    }
                }
            },
                                    ...
                                    if (void 0 !== p && null !== p && "null" !== p || (p = "-"), !r.colGroup) {
                                        ...
                                        function () {
                                            ...
                                            return r.toolbar ? i(t(r.toolbar).html() || "").render(n) : r.templet ?
                                                ...
                                                }() : r.renderData ?//2018-11-29 zj 修改
                                                    function () {
                                                        return "function" == typeof r.renderData ? r.renderData(n, p) : i(t(r.templet).html() || String(p)).render(n)
                                                    }() : p
                                        }(), "</div></td>"].join("");
                                        o.push(v),
            ...
            F.prototype.events = function () {
                ...
                a.elem.on("click", 'input[name="layTableCheckbox"]+', function () {
                    ...
                    if (a.config.isremember) {
                        ...
                        if (obj.checked == true) {
                            if (obj.type == 'one') {
                                obj.d.cache.temp.push(obj.data);
                            } else {
                                for (var i = 0; i < obj.d.cache.table.length; i++) {
                                    obj.d.cache.temp.push(obj.d.cache.table[i]);
                                }
                            }
                        } else {
                            if (obj.type == 'one') {
                                for (var i = 0; i < obj.d.cache.temp.length; i++) {
                                    if (obj.d.cache.temp[i] == obj.data) {
                                        obj.d.cache.temp = obj.d.cache.temp.remove(i);
                                    }
                                }
                                var k = obj.d.cache.temp.length
                                while (k--) {
                                    if (obj.d.cache.temp[k][a.config.tempid] == obj.data[a.config.tempid]) {
                                        obj.d.cache.temp = obj.d.cache.temp.remove(k);
                                    }
                                }
                            } else {
                                var k = obj.d.cache.temp.length
                                while (k--) {
                                    for (var j = 0; j < obj.d.cache.table.length; j++) {
                                        var oo = obj.d.cache.temp[k];
                                        if (oo == null) {
                                            continue;
                                        }
                                        if (obj.d.cache.temp[k][a.config.tempid] == obj.d.cache.table[j][a.config.tempid]) {
                                            obj.d.cache.temp = obj.d.cache.temp.remove(k);
                                        }
                                    }
                                }
