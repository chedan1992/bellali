...
namespace QINGUO.Common
{
    ...
    public  class StringHelper
    {
        ...
        /// 返回新HTML
        /// </summary>
        /// <param name="sHtmlText">html内容</param>
        /// <param name="scriptstr">js内容</param>
        /// <returns></returns>
        public static string GetNewHtml(string sHtmlText, out string scriptstr)
        {
            string newhtml = sHtmlText;
            scriptstr = "";
            string[] strimg = GetHtmlImageList(sHtmlText); 
            if (strimg.Length > 0)
            {
                string[] strimgsrc = GetHtmlImageSrcList(sHtmlText);
                scriptstr = "<script type=\"text/javascript\">  $(document).ready(function() { ";
                for (int i = 0; i < strimg.Length; i++)
                {
                    string oldstr = strimg[i];
                    string newstr = "<a id=\"example" + i + "\" href=\"" + strimgsrc[i] + "\" ><img alt=\"example4\"  src=\"" + strimgsrc[i] + "\" /></a>";
                    newhtml = newhtml.Replace(oldstr, newstr);
                    scriptstr += " $(\"a#example" + i + "\").fancybox(); ";
                }
                scriptstr += " }); </script>";
            }
            return newhtml;
