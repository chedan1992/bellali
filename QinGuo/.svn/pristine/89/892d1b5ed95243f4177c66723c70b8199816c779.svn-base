...
namespace QINGUO.Common
{
    public class PagerUtil
    {
        ...
        public string Render()
        {
            ...
            //计算总页数，如果是30条，则是3页，31条也是4页，29条也是3页，因此是
            //天花板运算Ceiling
            double tempCount = TotalCount / PageSize;//临时的页数
            int pageCount = (int)Math.Ceiling(tempCount);//获取总的页数

            //计算显示的页码数（当总页码大于MaxPagerCount）的起始页码
            int visibleStart = CurrentPageIndex - MaxPagerCount / 2;//只显示总页数的一半,5页显示3页,10页显示5页
            if (visibleStart < 1)//如果总页数小于了1,那么默认为只有一页
            {
                visibleStart = 1;
            }
