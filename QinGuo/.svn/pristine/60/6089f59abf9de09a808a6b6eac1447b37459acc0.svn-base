...
namespace WebApp.Controllers
{
    ...
    public class ShareApiController : ApiController
    {
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("api/register/{phone}/{pwd}/{MobileCode}/{code}/{PaltForm}")]
        public MyJsonResult<string> register(string phone, string pwd, string MobileCode, string code = "102913", int PaltForm = 3)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("api/getAAShare/{userid}")]
        public MyJsonResult<List<ModAAShare>> getAAShare(string userid)
        {
            MyJsonResult<List<ModAAShare>> jsonResult = new MyJsonResult<List<ModAAShare>>();
            try
            {
                ...
                if (jsonResult.success)
                {
                    BllAAShare bll = new BllAAShare();
                    jsonResult.data = bll.getUserId(userid);
                    jsonResult.msg = "获取成功！";
                }
            }
            catch (Exception ex)
        ...
        /// <param name="img">广告图</param>
        /// <param name="url">链接</param>
        /// <param name="isshow">是否开启</param>
        /// <returns></returns>
        [HttpPost]
        [Route("api/saveAAShare/{userid}/{token}")]
        public MyJsonResult<string> saveAAShare(string userid, string token, string id = "", string img = "", string url = "", bool isshow = false, int type = 1)
        {
            MyJsonResult<string> jsonResult = new MyJsonResult<string>();
            try
            {
                ...
                if (jsonResult.success && string.IsNullOrEmpty(token))
                {
                    jsonResult.success = false;
                    jsonResult.msg = "token不能为空！";
                }
                var mytoken = HttpContext.Current.Session["token"] != null ? HttpContext.Current.Session["token"].ToString() : "";
                if (jsonResult.success && mytoken != token)
                {
                    jsonResult.errorCode = 50;
                    jsonResult.success = false;
                    jsonResult.msg = "token已经失效，请重新登录！";
                }

                if (jsonResult.success)
                {
                    ModAAShare mode = new ModAAShare();
                    mode.CreaterId = userid;
                    mode.Img = img;
                    mode.Url = url;
                    mode.IsShow = isshow;
                    mode.CreateTime = DateTime.Now;
                    mode.Status = (int)FlowEnum.待审核;
                    mode.Type = type;
                    mode.Id = string.IsNullOrEmpty(id) ? Guid.NewGuid().ToString() : id;
                    if (string.IsNullOrEmpty(id) && new BllAAShare().Insert(mode) > 0)
                    {
                        jsonResult.data = mode.Id;
                        jsonResult.msg = "保存成功！";
                    }
                    else if (!string.IsNullOrEmpty(id) && new BllAAShare().Update(mode) > 0)
                    {
                        jsonResult.data = mode.Id;
                        jsonResult.msg = "保存成功！";
                    }
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("api/clickAShare/{id}")]
        public MyJsonResult<string> clickAShare(string id)
        {
            MyJsonResult<string> jsonResult = new MyJsonResult<string>();
            try
            {
                ...
                {
                    jsonResult.success = false;
                    jsonResult.msg = "广告id不能为空！";
                }

                if (jsonResult.success)
                {
                    ModAAShare mode = new BllAAShare().LoadData(id);
                    if (mode != null)
                    {
                        mode.Clicknum++;
                        if (new BllAAShare().Update(mode) > 0)
                        {
                            jsonResult.msg = "保存成功！";
                        }
                        else
        ...
        /// <param name="type">type==0 默认查询 type==1 城市查询</param>
        /// <returns></returns>
        [HttpGet]
        [Route("api/getNews")]
        public MyJsonResult<List<ModEDynamic>> getNews(string dircid, string userid, string search, int pagesize = 20, int pageindex = 1, int type = 0)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(dircid))
                {
                    ...
                    {
                        BllSysHatcity bllSysHatcity = new BllSysHatcity();
                        var city = bllSysHatcity.QuerCityName(dircid);
                        if (city != null)
                        {
                            searcht.AddCondition("CityId = '" + city.Code + "'");
                        }
                    }
                    else
                    {
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("api/getcollection/{userid}")]
        public MyJsonResult<List<ModEDynamic>> getcollection(string userid, int pagesize = 20, int pageindex = 1)
        {
            MyJsonResult<List<ModEDynamic>> jsonResult = new MyJsonResult<List<ModEDynamic>>();
            try
            {
                ...
                if (jsonResult.success)
                {
                    ...
                    if (!string.IsNullOrEmpty(userid))
                    {
                        search.AddCondition("c.CreaterId='" + userid + "'");
                    }

                    jsonResult.msg = "获取成功！";
                    var r = bllSysCollection.getEDynamicSearch(search).Items;
        ...
        /// </summary>
        /// <param name="userid">用户id</param>
        /// <param name="collid">新闻id</param>
        /// <returns></returns>
        [HttpPost]
        [Route("api/savecollection/{userid}/{collid}")]
        public MyJsonResult<string> savecollection(string userid, string collid)
        {
            MyJsonResult<string> jsonResult = new MyJsonResult<string>();
            try
            {
                ...
                if (jsonResult.success && string.IsNullOrEmpty(collid))
                {
                    jsonResult.success = false;
                    jsonResult.msg = "收藏id不能为空！";
                }
                if (jsonResult.success)
                {
                    var v = bllSysCollection.Exit(userid, collid, CollectionEnum.新闻收藏);
                    if (v != null)//取消收藏
                    {
                        if (bllSysCollection.Delete(v.Id) > 0)
                        {
                            jsonResult.errorCode = 2;
                            jsonResult.msg = "取消收藏成功！";
                        }
                        else
                        {
                            jsonResult.success = false;
                            jsonResult.msg = "取消收藏失败！";
                        }
                    }
                    else
                    {
                        ModSysCollection mode = new ModSysCollection();
                        mode.Id = Guid.NewGuid().ToString();
                        mode.CollId = collid;
                        mode.CreaterId = userid;
                        mode.CreateTime = DateTime.Now;
                        mode.Status = (int)StatusEnum.正常;
                        mode.Type = (int)CollectionEnum.新闻收藏;
                        if (bllSysCollection.Insert(mode) > 0)
                        {
                            jsonResult.errorCode = 1;
                            jsonResult.msg = "收藏成功！";
                        }
        ...
        /// <returns>mweb_url</returns>
        [HttpPost]
        [Route("api/h5pay/{userid}/{token}")]
        public MyJsonResult<string> h5pay(string userid, string token)
        {
            MyJsonResult<string> jsonResult = new MyJsonResult<string>();
            try
            {
                ...
                if (jsonResult.success)
                {
                    ...
                    var user = bllSysMaster.Get(userid);
                    if (user == null)
                    {
                        jsonResult.success = false;
                        jsonResult.msg = "用户不存在！";
                    }
                    else if (user != null && user.Status != (int)StatusEnum.正常)
                    {
                        jsonResult.success = false;
                        jsonResult.msg = "用户不存在或停用！";
                    }
                    else if (user != null)
                    {
                        List<ModOrderUserMoneyRecord> r = bll.QueryAll(user.Id).Where(c => c.Status == 2).ToList();//查询是否是会员
                        if (r != null)
                ...
                if (jsonResult.success)
                {
                    ModOrderUserMoneyRecord r = bll.QueryAll(userid).Find(c => c.Status == 1);//查询是否是会员
                    string orderid = "";
                    if (r != null)
                    {
                        r.MoneyNum = Convert.ToDecimal(0.01);
                        r.CreateTime = DateTime.Now;
                        if (bll.Update(r) <= 0)//下单
                        {
                            jsonResult.success = false;
                        }
                        orderid = r.Id;
                    ...
                    {
                        //获取微信  用户
                        ModOrderUserMoneyRecord mod = new ModOrderUserMoneyRecord();
                        mod.Id = Guid.NewGuid().ToString();
                        mod.MoneyNum = Convert.ToDecimal(0.01);
                        mod.UserId = userid;
                        mod.MoneyType = -1;
                        mod.CreateTime = DateTime.Now;
                        mod.BankName = "微信";
                        mod.Remark = "会员支付";
                        mod.Status = 1;//未支付
                        if (bll.Insert(mod) <= 0)//下单
                        {
                            jsonResult.success = false;
                        }
                        orderid = mod.Id;
                    }
                    if (jsonResult.success)//下单成功 获取预支付编号
                    {
                        WxPayData data = new WxPayData();
                        data.SetValue("body", "会员支付");
                        data.SetValue("out_trade_no", orderid);//订单号
                        data.SetValue("total_fee", 0.01.ToString()); //金额,以分为单位
                        data.SetValue("spbill_create_ip", HttpContext.Current.Request.UserHostAddress);//IP
                        data.SetValue("notify_url", "http://" + HttpContext.Current.Request.Url.Host + "/Webcode/view/msg.html"); //回调地址
                        data.SetValue("trade_type", "MWEB");//这个不可以改。固定为Mweb
                        //data.SetValue("sign", data.CreateMd5Sign("key", wxConfig.key));
                        //packageReqHandler.SetParameter("nonce_str", Senparc.Weixin.MP.TenPayLibV3.TenPayV3Util.GetNoncestr());

                        var reustl = WxPayApi.UnifiedOrder(data);
                        if (reustl.GetValue("return_code").ToString() == "SUCCESS")
                        {
                            jsonResult.msg = "下单成功！";
                            jsonResult.data = reustl.GetValue("prepay_id").ToString();//预支付编号
                            LogErrorRecord.ErrorFormat("mweb_url！：{0}", reustl.GetValue("mweb_url"));
                        }
                        else
                        {
                            jsonResult.success = false;
                            jsonResult.msg = "获取预支付编号失败！";
                            LogErrorRecord.ErrorFormat("获取预支付编号失败！：{0}", reustl.GetValue("return_code"));
                        }
                    }
                    else
                    {
                        jsonResult.msg = "下单失败！";
                        LogErrorRecord.ErrorFormat("下单失败！：{0}", orderid);
                    }
        ...
        [HttpGet]
        [Route("api/h5pay/{cid}")]
        public MyJsonResult<string> h5pay(string cid)
        {
            MyJsonResult<string> jsonResult = new MyJsonResult<string>();
            try
            {
                if (jsonResult.success && string.IsNullOrEmpty(cid))
                {
                    jsonResult.success = false;
                    jsonResult.msg = "公司id不能为空！";
                }
                if (jsonResult.success)
                {
                    BllSysCompany bllSysCompany = new BllSysCompany();
                    var modl = bllSysCompany.LoadData(cid);
                    if (modl != null)
                    {
                        jsonResult.data = modl.ReegistMoney.ToString();
